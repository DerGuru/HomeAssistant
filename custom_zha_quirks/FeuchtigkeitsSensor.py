...
from zhaquirks.tuya.tuya_trv import (
    TuyaThermostatV2,
...
class NoManufTimeTuyaMCUCluster(TuyaMCUCluster):
    """Tuya MCU Cluster with set_time override and robust tz handling.

    zhaquirks' default handler mixes tz-aware and tz-naive datetimes on some versions.
    We override it and compute both UTC and local epoch seconds consistently.
    """

    # Keep these as tz-aware UTC epoch for our own calculations
    set_time_offset = datetime.datetime(1970, 1, 1, tzinfo=datetime.UTC)

    # Deepcopy required to override 'set_time' command definition
    ...
    def handle_set_time_request(self, *args):
        """Handle device request for time and respond with UTC + local seconds since epoch."""
        now_utc = datetime.datetime.now(datetime.UTC)
        utc_seconds = int((now_utc - self.set_time_offset).total_seconds())
    
        local_tz = datetime.datetime.now().astimezone().tzinfo or datetime.UTC
        now_local = datetime.datetime.now(local_tz)
        local_epoch = datetime.datetime(1970, 1, 1, tzinfo=local_tz)
        local_seconds = int((now_local - local_epoch).total_seconds())
    
        # TuyaTimePayload behaves like a list/sequence on your zhaquirks version
        payload = TuyaTimePayload([utc_seconds, local_seconds])
    
        self.command(TUYA_SET_TIME, time=payload)
